...
namespace Trema::View
{
    ...
    {
    public:
        Checkbox(std::shared_ptr<IGuiElement> parent, std::string name);
        ~Checkbox();
        void AddOnCheckListener(std::string name, std::function<void(const Checkbox& checkbox, bool checked)> listener);
        void Show() override;

        static std::shared_ptr<Checkbox> CreateCheckbox(std::shared_ptr<IGuiElement> parent, std::string name);
