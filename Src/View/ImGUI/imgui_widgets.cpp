...
// So e.g. an integer SliderFloat between INT_MAX-10 and INT_MAX will fail, but an integer SliderFloat between INT_MAX/2-10 and INT_MAX/2 will be ok.
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Tabbing or CTRL-clicking on SliderFloat turns it into an input box
    const bool hovered = ItemHoverable(frame_bb, id);
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
...
// Note that Drag/SliderFloat functions are only forwarding the min/max values clamping values if the ImGuiSliderFlags_AlwaysClamp flag is set!
